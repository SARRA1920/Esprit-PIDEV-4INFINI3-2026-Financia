...
import { SignatureContratComponent } from '../../admin/signature-contrat.component';

@Component({
  selector: 'app-espace-client',
  standalone: true,
  imports: [ReactiveFormsModule, DecimalPipe, DatePipe, NgClass, RouterLink, SignatureContratComponent],
...
export class EspaceClientComponent implements OnInit {
  ...
  /** Toutes les échéances triées par date croissante. */
  remboursementsSorted(): Remboursement[] {
    return [...(this.remboursements ?? [])].sort((a, b) =>
      String(a.dueDate ?? '').localeCompare(String(b.dueDate ?? ''))
    );
  }

  paidRemboursementsCount(): number {
    return (this.remboursements ?? []).filter(r => String(r.status ?? '').toUpperCase() === 'PAID').length;
  }

  pendingRemboursementsCount(): number {
  ...
  lateRemboursementsCount(): number {
    const today = this.todayIsoLocal();
    return (this.remboursements ?? []).filter(r => {
      const s = String(r.status ?? '').toUpperCase();
      return s !== 'PAID' && !!r.dueDate && String(r.dueDate) < today;
    }).length;
  }

  historyRowClass(r: Remboursement): string {
    const s = String(r.status ?? '').toUpperCase();
    const today = this.todayIsoLocal();
    if (s === 'PAID') return 've-hrow--paid';
    if (!!r.dueDate && String(r.dueDate) < today) return 've-hrow--late';
    return 've-hrow--pending';
  }

  historyDotClass(r: Remboursement): string {
    const s = String(r.status ?? '').toUpperCase();
    const today = this.todayIsoLocal();
    if (s === 'PAID') return 've-dot--paid';
    if (!!r.dueDate && String(r.dueDate) < today) return 've-dot--late';
    return 've-dot--pending';
  }

  payPillClass(r: Remboursement): string {
    const s = String(r.status ?? '').toUpperCase();
    const today = this.todayIsoLocal();
    if (s === 'PAID') return 'is-paid';
    if (!!r.dueDate && String(r.dueDate) < today) return 'is-failed';
    return 'is-pending';
  }
    ...
    this.credits.getBlockingInfo(u.idUser).subscribe({
      next: (info) => {
        ...
        if (
          ...
          this.loadContrat(this.blockingCredit.id);
  ...
  private loadContrat(creditId: number): void {
    this.contratLoading = true;
    this.contratService.getContratByCreditId(creditId).subscribe({
      next: (c) => { this.contrat = c; this.contratLoading = false; },
      error: () => { this.contrat = null; this.contratLoading = false; },
    });
  }

  /** True si le crédit est dans un état où le contrat doit être signé. */
  shouldShowSignature(): boolean {
    const status = String(this.blockingCredit?.status ?? '').toUpperCase();
    return (status === 'APPROVED' || status === 'ACTIVE') && !!this.contrat?.id;
  }

  private loadRemboursements(creditId: number): void {    this.remboursementsLoading = true;
    this.remboursementsError = '';
    ...
    this.credits.acceptOffer(c.id, u.idUser).subscribe({
      next: (cred) => {
        ...
        if (cred.id) {
          ...
          this.loadContrat(cred.id);
        }
        /* Ne pas appeler refreshBlockingCredit() ici : un GET trop rapide peut précéder le commit côté serveur
         * ou renvoyer un état incohérent ; le POST renvoie déjà le crédit à jour (APPROVED). */
      },
      error: (e: Error) => {
