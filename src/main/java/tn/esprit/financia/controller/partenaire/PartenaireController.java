...
import tn.esprit.financia.service.partenaire.IPartenaireService;
import tn.esprit.financia.service.partenaire.IPartenaireFondService;
import tn.esprit.financia.service.partenaire.EmailValidationService;
...
public class PartenaireController {
    ...
    public ResponseEntity<?> createPartenaire(@RequestBody Partenaire partenaire) {
        // Validate email for fraud
        if (partenaire.getEmail() != null) {
            EmailValidationService.EmailValidationResult validation = 
                emailValidationService.validateEmail(partenaire.getEmail(), partenaire.getName());
            ...
            if (!validation.isValid()) {
                return ResponseEntity.badRequest().body(Map.of(
                    ...
                    "suspicionScore", validation.getSuspicionScore()
                ));
            }
            
            // If suspicious but valid, add warning to response
            if (validation.getSuspicionScore() > 0) {
                Partenaire savedPartenaire = partenaireService.save(partenaire);
                return ResponseEntity.status(HttpStatus.CREATED).body(Map.of(
                    "partenaire", savedPartenaire,
                    "warning", validation.getMessage(),
                    "emailValidation", Map.of(
                        "level", validation.getLevel(),
                        "suspicionScore", validation.getSuspicionScore()
                    )
                ));
